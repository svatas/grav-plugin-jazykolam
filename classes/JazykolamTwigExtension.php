...
class JazykolamTwigExtension extends AbstractExtension
{
    ...
    protected $config;
    /** @var string|null */
    protected static $localeOverride = null; // per-request override
    ...
    /** Public API: set per-render locale override */
    ...
    public function monthFilter($value, string $form = 'long', ?string $locale = null): string
    {
        ...
        if (isset($months[$form][$month])) return (string)$months[$form][$month];
    ...
    public function timeFilter($value, ?string $locale = null, $now = null): string
    {
        ...
        $unit = 'second'; $count = 1;
        foreach ($units as [$u, $sec]) { if ($abs >= $sec) { $unit = $u; $count = (int)floor($abs/$sec); break; } }
    ...
    public function autoT($value, ...$params)
    {
        ...
        // Try locale-aware lookup for ICU-lite patterns stored as plain strings
        ...
        // Fallback to Grav translate (active language)
    ...
    {
        // If locale override provided, try to fetch raw translation for that locale
        if ($locale) {
            $raw = $this->translateRaw($key, $locale);
            if ($raw !== $key && is_string($raw)) {
                return $this->language->translate([$key], $params); // let Grav do replacements
            }
        }
    ...
    {
        // Best effort: reach into Language class data
        try {
            $lang = $this->language;
            $data = $lang->getTranslation($locale); // may not exist in all Grav versions; ignore errors
            if (is_array($data)) {
                $parts = explode('.', $key);
                $val = $data;
                foreach ($parts as $p) {
