...
class JazykolamTwigExtension extends AbstractExtension
{
    ...
    // -------------------- Filters (shortened for brevity here; same as 1.3.3) --------------------
    public function pluralFilter($count, $forms, ?string $locale = null): string
    {
        ...
        if (is_string($forms)) {
            ...
            $keyUsed = '{map}';
            $map = $forms;
            if (array_keys($forms) === range(0, count($forms)-1)) {
                ...
                if (isset($map[$category])) $out=$this->interpolate($map[$category],$count);
                elseif (isset($map['other'])) $out=$this->interpolate($map['other'],$count);
            }
        } else { $out=(string)$forms; $keyUsed='{string}'; }
        return $this->maybeDebugWrap($out, ['source'=>'plural','key'=>$keyUsed,'locale'=>$locale,'meta'=>['category'=>$category,'count'=>$count]]);
    }

    public function monthFilter($value, string $form = 'long', ?string $locale = null): string
    {
        $locale = $locale ?: $this->getActiveLocale();
        $m = $this->normalizeMonth($value); if ($m<1||$m>12) return '';
        $key = sprintf('JAZYKOLAM.MONTH.%s.%d', strtoupper($form), $m);
        $translated = $this->translate($key, [], $locale);
        if ($translated !== $key) $out = (string)$translated; else $out = '';
        return $this->maybeDebugWrap($out ?: (string)$m, ['source'=>'month','key'=>$key,'locale'=>$locale,'meta'=>['form'=>$form,'month'=>$m]]);
    }

    public function timeFilter($value, ?string $locale = null, $now = null): string
    {
        $locale = $locale ?: $this->getActiveLocale();
        $nowTs=$this->toTimestamp($now??'now'); $delta=$this->valueToDeltaSeconds($value,$nowTs); $abs=abs($delta);
        if ($abs < 45) {
            $key='JAZYKOLAM.RELATIVE.NOW'; $t=$this->translate($key, [], $locale);
            $out = ($t !== $key) ? $t : 'just now';
            return $this->maybeDebugWrap($out, ['source'=>'time','key'=>$key,'locale'=>$locale,'meta'=>['delta'=>$delta]]);
        }
        ...
        $key = sprintf('JAZYKOLAM.RELATIVE.%s.%s.other', strtoupper($direction), strtoupper($unit));
        $t=$this->translate($key,[$count],$locale); $out=$this->interpolate($t,$count);
        return $this->maybeDebugWrap($out, ['source'=>'time','key'=>$key,'locale'=>$locale,'meta'=>['direction'=>$direction,'unit'=>$unit,'count'=>$count,'delta'=>$delta]]);
    }
    ...
    public function autoT($value, ...$params)
    {
        ...
        $key = (string)$value; $raw = $this->translateRaw($key, $locale);
        $out = null;
        ...
        return $this->maybeDebugWrap($out, ['source'=>'autoT','key'=>$key,'locale'=>$locale,'meta'=>['count'=>$count]]);
    }
    ...
    public function debugPanelFunction(): string
    {
        ...
        $rows=''; foreach($items as $i){ $rows.=sprintf('<tr><td>%s</td><td>%s</td><td>%s</td><td><code>%s</code></td></tr>',
            htmlspecialchars($i['source']??'',ENT_QUOTES), htmlspecialchars($i['locale']??'',ENT_QUOTES), htmlspecialchars($i['key']??'',ENT_QUOTES), htmlspecialchars(mb_strimwidth($i['out']??'',0,120,'…'),ENT_QUOTES)); }
    ...
    public function debugConsoleFunction(): string
    {
        ...
        return '<script>window.__JAZYKOLAM_LOG__='+ $log +';(function(){try{var d=window.__JAZYKOLAM_LOG__;if(Array.isArray(d)&&d.length){console.group("Jazykolam DEBUG");console.table(d.map(function(x){return {source:x.source,locale:x.locale,key:x.key,out:x.out};}));console.groupEnd();}}catch(e){}})();</script>';
    }
    ...
    protected function maybeDebugWrap(string $out, array $info): string
    {
        ...
        $attrs = sprintf('data-jl-source="%s" data-jl-key="%s" data-jl-locale="%s"',
            htmlspecialchars($entry['source'], ENT_QUOTES), htmlspecialchars($entry['key'], ENT_QUOTES), htmlspecialchars($entry['locale'], ENT_QUOTES));
        ...
        if ($mode === 'inline') {
            return sprintf('<span class="jl-debug" %s>%s%s%s<sup class="jl-badge" style="font-size:9px;background:#f39c12;color:#000;padding:1px 3px;border-radius:3px;margin-left:2px;">%s</sup></span>',
                $attrs, htmlspecialchars($prefix, ENT_QUOTES), $out, htmlspecialchars($suffix, ENT_QUOTES), htmlspecialchars($badge, ENT_QUOTES));
        }
